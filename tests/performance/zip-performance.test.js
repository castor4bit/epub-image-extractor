...
// 将来のfflate実装との比較用インターフェース
export interface PerformanceMetrics {
  openAndListTime: number;
  containerReadTime: number;
  fullProcessingTime: number;
  memoryPerZip: number;
}

export function recordPerformanceBaseline(): PerformanceMetrics {
  const testEpubPath = path.join(__dirname, '../fixtures/test.epub');
  const iterations = 100;
  ...
  const openAndListTime = (performance.now() - start) / iterations;
  ...
  const containerReadTime = (performance.now() - start) / iterations;
  ...
  const fullProcessingTime = (performance.now() - start) / 10;
  ...
  const memoryPerZip = (process.memoryUsage().heapUsed - initialMemory) / 10 / 1024 / 1024;
