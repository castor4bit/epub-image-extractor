...
describe('ZIP Reader Implementations', () => {
  const testEpubPath = path.join(__dirname, '../../../../../tests/fixtures/test.epub');
  const implementations: { name: string; createReader: () => IZipReader }[] = [
    { name: 'AdmZipReader', createReader: () => new AdmZipReader() },
    { name: 'FflateReader', createReader: () => new FflateReader() },
  ];
  
  implementations.forEach(({ name, createReader }) => {
    describe(name, () => {
  ...
  describe('Implementation Compatibility', () => {
    it('should produce identical results between implementations', async () => {
      const admReader = new AdmZipReader();
      const fflateReader = new FflateReader();
      
      await admReader.open(testEpubPath);
      await fflateReader.open(testEpubPath);
      
      // エントリ数が同じ
      const admEntries = admReader.getEntries();
      const fflateEntries = fflateReader.getEntries();
      expect(admEntries.length).toBe(fflateEntries.length);
      
      // container.xmlの内容が同じ
      const containerEntry = 'META-INF/container.xml';
      const admContainer = admReader.readAsText(admReader.getEntry(containerEntry)!);
      const fflateContainer = fflateReader.readAsText(fflateReader.getEntry(containerEntry)!);
      expect(admContainer).toBe(fflateContainer);
