...
import {
  createLargeTestEpub,
  saveLargeTestEpub,
} from '../../../../tests/helpers/create-large-epub';
...
describe('zipHandler with large files', () => {
  ...
  it('should handle 50MB EPUB file efficiently', async () => {
    ...
    const imageEntries = entries.filter(
      (entry) => !entry.isDirectory && entry.name.match(/\.(jpg|jpeg|png|webp)$/i),
    );
  ...
  it('should handle concurrent access to large files', async () => {
    ...
    const promises = Array.from({ length: 5 }, async (_, index) => {
      ...
      // ランダムに10個の画像を読む
      const randomImages = imageEntries.sort(() => Math.random() - 0.5).slice(0, 10);
