...
<package xmlns="http://www.idpf.org/2007/opf" version="3.0">
  ...
  test('同じEPUBファイルを2回処理した場合の動作', async () => {
    ...
    // 1回目の処理
    const results1 = await processEpubFiles(
      [testEpubPath],
      outputDir,
      onProgress,
      1
    ...
    // 2回目の処理
    const results2 = await processEpubFiles(
      [testEpubPath],
      outputDir,
      onProgress,
      1
  ...
  test('異なるEPUBファイルで同じ名前の場合', async () => {
    ...
    mockZip2.addFile('META-INF/container.xml', Buffer.from(`<?xml version="1.0" encoding="UTF-8"?>
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    <rootfile full-path="OEBPS/content.opf" media-type="application/oebps-package+xml"/>
  </rootfiles>
    ...
    mockZip2.addFile('OEBPS/content.opf', Buffer.from(`<?xml version="1.0" encoding="UTF-8"?>
<package xmlns="http://www.idpf.org/2007/opf" version="3.0">
  <metadata>
  ...
  <spine>
    ...
    mockZip2.addFile('OEBPS/page1.xhtml', Buffer.from('<html><body><img src="different.png"/></body></html>'));
