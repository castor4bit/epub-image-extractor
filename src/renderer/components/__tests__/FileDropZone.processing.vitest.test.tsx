...
describe('FileDropZone - Processing State', () => {
  const mockHandlers = {
    onDragEnter: vi.fn(),
    onDragOver: vi.fn(),
    onDragLeave: vi.fn(),
  ...
  it('should apply disabled class when processing', () => {
    const { rerender } = render(
      <FileDropZone isDragging={false} isProcessing={false} {...mockHandlers} />,
    );
    ...
    // 処理中状態
    rerender(<FileDropZone isDragging={false} isProcessing={true} {...mockHandlers} />);
    expect(dropZone).toHaveClass('disabled');
  });

  it('should block drag events when processing', () => {
    render(<FileDropZone isDragging={false} isProcessing={true} {...mockHandlers} />);
  ...
  it('should allow drag events when not processing', () => {
    render(<FileDropZone isDragging={false} isProcessing={false} {...mockHandlers} />);
